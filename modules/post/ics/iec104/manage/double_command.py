...
class Module(Post, Ics_scanner_client, Iec104SessionMixin):
    __info__ = {
        "name": "IEC104 double command",
        "description": (
            "Sends an IEC 60870-5-104 double command (C_DC_NA_1). "
    ...
    value = OptInteger(2, "DCO value (1=OFF, 2=ON)", True)
    select = OptBool(False, "Select before execute", False)
    ...
    def run(self):
        ...
        if bool(self.dry_run):
            print_success(
                f"Dry run — would C_DC_NA_1 IOA={ioa} dco={value} select={select}"
            )
            return True

        if not bool(self.confirm):
            print_error("Set confirm=true to send IEC104 double command (lab only)")
            return False
        ...
        try:
            if not client.startdt():
                ...
                return False
            if client.double_command(ioa, value, select):
                print_success(f"Double command sent IOA={ioa} dco={value}")
                return True
            print_error("Double command failed")
            return False
        finally:
            if owns_client:
                client.close()
