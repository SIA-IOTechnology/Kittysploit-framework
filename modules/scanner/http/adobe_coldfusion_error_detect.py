...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'adobe', 'coldfusion', 'tech'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://twitter.com/PhilippeDelteil/status/1418622775829348358'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/_something_.cfm', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('ColdFusion documentation',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Adobe ColdFusionor detected",
