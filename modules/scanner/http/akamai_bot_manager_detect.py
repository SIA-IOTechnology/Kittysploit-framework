...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'akamai', 'bot', 'security'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        header_regexes = ('Set-Cookie:\\s*_abck=', 'Set-Cookie:\\s*bm_sv=', 'Set-Cookie:\\s*ak_bmsc=', 'Set-Cookie:\\s*bm_sz=', 'Set-Cookie:\\s*bm_mi=',)
        if any(re.search(rx, headers, 0) for rx in header_regexes):
            self.set_info(
                severity='info',
                reason="Akamai Bot Manager Protection detected",
                path='/',
            )
            return True
        return False
