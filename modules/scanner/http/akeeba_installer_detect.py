...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Akeeba Backup Installer - Exposure Detection',
        'description': 'Akeeba Backup is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'akeeba', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('Akeeba Backup Installer',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Akeeba Backup Installer - Exposure detected",
