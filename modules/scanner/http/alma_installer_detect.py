...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Alma Installation Exposure Detection',
        'description': 'Alma is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'alma', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_any = ('<title>Setup wizard | Alma Installation</title>',)
        header_any = ('text/html',)
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Alma Installation Exposure detected",
                path='/setup/start',
            )
            return True
        return False
