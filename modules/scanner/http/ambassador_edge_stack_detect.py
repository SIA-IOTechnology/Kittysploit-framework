#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Ambassador Edge Stack is a Kubernetes-native API Gateway that delivers the scalability, security, and simplici."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'ambassador', 'gateway'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('Ambassador Edge Stack</p>',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Ambassador Edge Stack detected",
                path='/',
            )
            return True
        return False
