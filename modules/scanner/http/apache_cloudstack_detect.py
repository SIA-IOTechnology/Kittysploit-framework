#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CloudStack is open-source Infrastructure-as-a-Service cloud computing software for creating, managing, and dep."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'apache', 'cloudstack'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('<title>Apache CloudStack</title>',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Apache CloudStack detected",
                path='/',
            )
            return True
        return False
