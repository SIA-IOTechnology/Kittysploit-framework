...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'apache', 'streampipes'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/streampipes-backend/api/openapi.json', '/assets/img/favicon/favicon.ico', '/'):
            r = self.http_request(method="GET", path=path, allow_redirects=True)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_markers = ('<title>apache streampipes', 'apache streampipes api',)
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="Apache StreamPipes detected",
                    path=path,
                )
                return True
        return False
