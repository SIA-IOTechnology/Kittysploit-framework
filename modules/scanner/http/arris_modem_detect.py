...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'arris', 'commscope'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/phy.htm', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="ARRIS Touchstone Telephony Modem - Panel Detect detected",
                path='/phy.htm',
            )
            return True
        return False
