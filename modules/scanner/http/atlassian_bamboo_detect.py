...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'bamboo', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/userlogin!doDefault.action?os_destination=%2Fstart.action', allow_redirects=False)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Atlassian Bamboo Login Panel detected",
