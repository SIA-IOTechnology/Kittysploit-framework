...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'iot', 'panel', 'edb'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.exploit-db.com/ghdb/7295'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<TITLE>C-more -- the best HMI presented by AutomationDirect</TITLE>',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='info',
                reason="AutomationDirect Panel detected",
                path='/index.html',
            )
            return True
        return False
