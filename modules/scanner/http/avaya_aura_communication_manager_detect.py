...
class Module(Scanner, Http_client):
    ...
    def run(self):
        path = '/cgi-bin/common/login/webLogin'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Avaya Aura', 'Communication Manager (CM)',)
        if all(m in body for m in body_all):
            self.set_info(
                severity='info',
                reason='Avaya Aura Communication Manager Login detected',
                path=path,
            )
            return True
        return False
