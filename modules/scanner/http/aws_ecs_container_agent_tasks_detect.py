...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'aws-ecs-container-agent-tasks Detection',
        'description': 'Aws container metadata content',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'misc', 'miscellaneous', 'aws', 'docker', 'ec2', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/v1/metadata', '/v1/tasks'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_any = ('arn:aws:ecs',)
            if (any(m in body for m in body_any)):
                self.set_info(
                    severity='info',
                    reason="aws-ecs-container-agent-tasks detected",
                    path=path,
                )
                return True
        return False
