...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Azure Resource Manager Template - File Exposure Detection',
        'description': 'Azure Resource Manager deploy file is disclosed.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'exposure', 'azure', 'files', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/azuredeploy.json', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('"$schema":', '"contentVersion":', '"parameters":',)
        header_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="Azure Resource Manager Template - File Exposure detected",
