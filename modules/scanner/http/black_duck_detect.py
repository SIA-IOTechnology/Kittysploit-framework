...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'blackduck', 'synopsys'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('ProtexLoginPage', 'CCLoginPage',)
        body_regexes = ('(?m)<title>Black Duck(.*)<\\/title>',)
        if any(m in body for m in body_markers) and any(re.search(rx, body, 0) for rx in body_regexes):
            self.set_info(
                severity='info',
                reason="Black Duck Login Panel detected",
                path='/',
            )
            return True
        return False
