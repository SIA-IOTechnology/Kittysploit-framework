...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'blazor', 'webassembly'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_markers = ('dotnet.wasm', '"entryassembly"', '"microsoft.jsinterop.webassembly.dll"',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Blazor WebAssembly detected",
