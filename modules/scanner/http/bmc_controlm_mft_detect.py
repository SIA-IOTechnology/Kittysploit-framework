...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'BMC Control-M MFT products was detected.',
    ...
    def run(self):
        ...
        body_any = ('file exchange</title>', 'main-b2b-icon.png', 'fileexchange.ico',)
        if any(m in body for m in body_any):
            self.set_info(
                severity='info',
                reason='BMC Control-M MFT Login Panel detected',
                path=path,
            )
            return True
        return False
