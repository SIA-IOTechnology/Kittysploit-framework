...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'BRAVIA Signage - Exposure Detection',
        'description': 'Bravia Signage is exposed.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'exposure', 'bravia', 'sony', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('BRAVIA Signage',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='medium',
                reason="BRAVIA Signage - Exposure detected",
                path='/#/settings',
            )
            return True
        return False
