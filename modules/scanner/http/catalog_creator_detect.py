...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'catalogcreater', 'panel'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/', '/index.php'):
            ...
            body_markers = ('powered by catalogcreator', 'content="catalogcreator',)
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="CATALOGcreator Page Login Panel detected",
                    path=path,
                )
                return True
        return False
