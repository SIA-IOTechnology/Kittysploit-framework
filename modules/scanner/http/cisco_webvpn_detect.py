...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'cisco', 'vpn'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/', '/webvpn.html'):
            r = self.http_request(method='GET', path=path, allow_redirects=True)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items()).lower()
            body_any = ('cisco', 'anyconnect', 'sslvpn service',)
            header_regexes = ('webvpncontext=00@.+', 'webvpn=',)
            if (any(m in body for m in body_any)) and (any(re.search(rx, headers, re.I) for rx in header_regexes)):
                self.set_info(
                    severity='info',
                    reason='Cisco WebVPN Panel detected',
                    path=path,
                )
                return True
        return False
