...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'cobbler', 'webserver', 'cobblerd'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/cobbler_web', allow_redirects=True)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Cobbler WebGUI Login Panel detected",
                path='/cobbler_web',
            )
            return True
        return False
