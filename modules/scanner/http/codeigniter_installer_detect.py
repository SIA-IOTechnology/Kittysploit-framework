...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Codeigniter Application Installer Exposure Detection',
        'description': 'Codeigniter Application is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'codeigniter', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('Codeigniter Application Installer',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Codeigniter Application Installer Exposure detected",
                path='/install/',
            )
            return True
        return False
