...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/config/default.json', '/config.json', '/config/config.json', '/credentials/config.json'):
            r = self.http_request(method='GET', path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            content_type = r.headers.get("Content-Type") or r.headers.get("content-type") or ""
            body_any = ('"api_keys":', '"aws":', '"server":', '"users":', '"password":', '"user"', '"path":',)
            ctype_any = ('text/html', 'text/javascript',)
