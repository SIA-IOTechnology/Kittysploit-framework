...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Contentify Installer Exposure Detection',
        'description': 'Contentify is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'contentify', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Contentify - Step 1', 'start the installation',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='high',
                reason="Contentify Installer Exposure detected",
                path='/install',
            )
            return True
        return False
