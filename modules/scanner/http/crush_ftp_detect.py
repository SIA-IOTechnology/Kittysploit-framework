...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'edb', 'crushftp'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/WebInterface/login.html', allow_redirects=False)
        ...
        if body_hit and any(m in headers for m in header_markers):
            self.set_info(
                severity='info',
                reason="CrushFTP WebInterface Panel detected",
