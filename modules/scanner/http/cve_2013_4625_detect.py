#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A cross-site scripting vulnerability in files/installer."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Plugin Duplicator < 0.4.5 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve2013', 'cve', 'seclists', 'wordpress', 'xss', 'wp-plugin', 'packetstorm', 'cory_lamle', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('Duplicator - WordPress Migration',)
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WordPress Plugin Duplicator < 0.4.5 - Cross-Site Scripting detected', path=path)
            return True
        return False
