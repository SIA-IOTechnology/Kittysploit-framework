...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/~user/%0D%0ASet-Cookie:crlfinjection', allow_redirects=False)
        ...
        if (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="Apache mod_userdir CRLF injection detected",
                path='/~user/%0D%0ASet-Cookie:crlfinjection',
            )
            return True
        return False
