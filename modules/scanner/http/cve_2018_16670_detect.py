...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2018', 'scada', 'plc', 'iot', 'disclosure', 'edb', 'circarlife', 'circontrol', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/services/user/values.xml?var=STATUS', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('<values><variable><id>', 'Reader.STATUS',)
        header_any = ('CirCarLife Scada',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="CirCarLife <4.3 - Improper Authentication detected",
                path='/services/user/values.xml?var=STATUS',
            )
            return True
        return False
