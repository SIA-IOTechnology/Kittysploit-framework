#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Kirona Dynamic Resource Scheduler is susceptible to information disclosure."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Kirona Dynamic Resource Scheduler - Information Disclosure Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2019', 'exposure', 'edb', 'packetstorm', 'kirona', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/osm/REGISTER.cmd', '/osm_tiles/REGISTER.cmd'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_all = ('DEBUGMAPSCRIPT=TRUE', '@echo off',)
            if (all(m in body for m in body_all)):
                self.set_info(
                    severity='medium',
                    reason="Kirona Dynamic Resource Scheduler - Information Disclosure detected",
                    path=path,
                )
                return True
        return False
