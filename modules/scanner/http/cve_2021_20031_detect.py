...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SonicWall SonicOS 7.0 - Open Redirect Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'sonicwall', 'redirect', 'edb', 'packetstorm', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('https://{{randstr}}.tld/auth.html', 'Please be patient as you are being re-directed',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="SonicWall SonicOS 7.0 - Open Redirect detected",
                path='/',
            )
            return True
        return False
