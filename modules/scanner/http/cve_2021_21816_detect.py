...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'D-Link DIR-3040 1.13B03 - Information Disclosure Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'dlink', 'exposure', 'router', 'syslog', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/messages', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('syslog:', '/etc_ro/lighttpd/www')
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="D-Link DIR-3040 1.13B03 - Information Disclosure detected",
                path='/messages',
            )
            return True
        return False
