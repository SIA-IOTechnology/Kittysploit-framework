...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Noptin < 1.6.5 - Open Redirect Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'wp', 'wpscan', 'wordpress', 'redirect', 'wp-plugin', 'noptin', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/?noptin_ns=email_click&to=https://interact.sh', allow_redirects=False)
        ...
        if (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="Noptin < 1.6.5 - Open Redirect detected",
                path='/?noptin_ns=email_click&to=https://interact.sh',
            )
            return True
        return False
