...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Prometheus - Open Redirect Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'prometheus', 'redirect', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/new/newhttp://interact.sh', allow_redirects=False)
        ...
        if (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="Prometheus - Open Redirect detected",
