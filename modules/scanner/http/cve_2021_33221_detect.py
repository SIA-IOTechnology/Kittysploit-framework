#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CommScope Ruckus IoT Controller is susceptible to information disclosure vulnerabilities because a 'service de."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'CommScope Ruckus IoT Controller - Information Disclosure Detection',
        ...
        'severity': 'critical',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'commscope', 'ruckus', 'debug', 'service', 'leak', 'seclists', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/service/v1/service-details', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('message', 'ok', 'data', 'dns', 'gateway',)
        header_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='critical',
                reason="CommScope Ruckus IoT Controller - Information Disclosure detected",
                path='/service/v1/service-details',
            )
            return True
        return False
