#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Specially crafted "X-Forwarded-Host" headers in combination with certain "allowed host" formats can cause the ."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Open Redirect in Host Authorization Middleware Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2021', 'seclists', 'redirect', 'rubyonrails', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="Open Redirect in Host Authorization Middleware detected",
                path='/',
            )
            return True
        return False
