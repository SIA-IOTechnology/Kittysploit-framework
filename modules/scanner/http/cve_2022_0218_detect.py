#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""The WP HTML Mail WordPress plugin is vulnerable to unauthorized access which allows unauthenticated attackers ."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'HTML Email Template Designer < 3.1 - Missing Authorization on Rest Route Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2022', 'wordpress', 'wp-plugin', 'codemiq', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/index.php?rest_route=/whm/v3/themesettings', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('"background":', '"footer":',)
        header_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="HTML Email Template Designer < 3.1 - Missing Authorization on Rest Route detected",
                path='/index.php?rest_route=/whm/v3/themesettings',
            )
            return True
        return False
