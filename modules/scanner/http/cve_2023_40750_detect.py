...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'PHPJabbers Yacht Listing Script v1.0 - Cross-Site Scripting Detection',
        'description': 'There is a Cross Site Scripting (XSS) vulnerability in the "action" parameter of index.php in PHPJabbers Yacht Listing Script v1.0.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2023', 'phpjabbers', 'yacht-listing', 'xss', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if (all(m in body for m in body_all)) and (any(m in content_type for m in ctype_any)):
            self.set_info(
                severity='medium',
                reason='PHPJabbers Yacht Listing Script v1.0 - Cross-Site Scripting detected',
                path=path,
            )
            return True
        return False
