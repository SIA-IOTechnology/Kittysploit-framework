#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PHPJabbers Fundraising Script v1."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'PHPJabbers Fundraising Script v1.0 - Cross-Site Scripting Detection',
        'description': 'PHPJabbers Fundraising Script v1.0 is vulnerable to Cross Site Scripting (XSS) via the "action" parameter of index.php.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2023', 'phpjabbers', 'fundraising', 'xss', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if (all(m in body for m in body_all)) and (any(m in content_type for m in ctype_any)):
            self.set_info(
                severity='medium',
                reason='PHPJabbers Fundraising Script v1.0 - Cross-Site Scripting detected',
                path=path,
            )
            return True
        return False
