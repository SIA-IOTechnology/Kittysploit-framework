#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""An SSRF (Server-Side Request Forgery) vulnerability exists in the gradio-app/gradio repository, allowing attac."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Gradio - Server Side Request Forgery Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2024', 'ssrf', 'oast', 'gradio', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if any(re.search(rx, headers) for rx in header_regexes):
            self.set_info(severity='medium', reason='Gradio - Server Side Request Forgery detected', path=path)
            return True
        return False
