...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'LearnDash LMS < 4.10.3 - Sensitive Information Exposure Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2024', 'wp', 'wp-plugin', 'wordpress', 'exposure', 'learndash', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/wp-json/wp/v2/sfwd-question', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('"id":', '"question_type":', '"points_total":',)
        header_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='medium',
                reason="LearnDash LMS < 4.10.3 - Sensitive Information Exposure detected",
                path='/wp-json/wp/v2/sfwd-question',
            )
            return True
        return False
