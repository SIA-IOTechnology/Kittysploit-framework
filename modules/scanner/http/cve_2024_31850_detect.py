#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A path traversal vulnerability exists in the Java version of CData Arc < 23."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'CData Arc < 23.4.8839 - Path Traversal Detection',
    ...
    def run(self):
        ...
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(severity='high', reason='CData Arc < 23.4.8839 - Path Traversal detected', path=path)
            return True
        return False
