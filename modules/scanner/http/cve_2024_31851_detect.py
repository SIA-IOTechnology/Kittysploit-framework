#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A path traversal vulnerability exists in the Java version of CData Sync < 23."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'CData Sync < 23.4.8843 - Path Traversal Detection',
    ...
    def run(self):
        ...
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(severity='high', reason='CData Sync < 23.4.8843 - Path Traversal detected', path=path)
            return True
        return False
