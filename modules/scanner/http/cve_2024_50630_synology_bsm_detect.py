#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Synology Drive Server unauthenticated access token leak on BeeStation (Synology-SA-24:21)."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Synology BeeStation Drive Server - Auth Bypass Token Leak (CVE-2024-50630)',
        'description': (
            'Synology Drive Server on BeeStation (BSM) allows unauthenticated attackers to read '
            'Drive logs via RedirectURI and obtain user access tokens via '
        ...
        'severity': 'critical',
        'tags': [
            'web', 'scanner', 'cve', 'cve2024', 'synology', 'beestation', 'bsm', 'drive',
    ...
    def run(self):
        ...
        self.set_info(
            severity='critical',
            reason=f'Synology BeeStation Drive CVE-2024-50630 token leak ({len(tokens)} user(s))',
            path=path,
