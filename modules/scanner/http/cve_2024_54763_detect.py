#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""An access control issue in the component /login/hostinfo."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'ipTIME A2004 - Unauthorized Access Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2024', 'iptime', 'router', 'unauth', 'exposure', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if (any(m in content_type for m in ctype_any)) and (any(re.search(rx, body) for rx in body_regexes)):
            self.set_info(
                severity='medium',
                reason='ipTIME A2004 - Unauthorized Access detected',
                path=path,
            )
            return True
        return False
