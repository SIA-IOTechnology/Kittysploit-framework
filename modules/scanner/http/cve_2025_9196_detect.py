#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""The Trinity Audio Text to Speech AI audio player to convert content into audio plugin for WordPress is vulnera."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Trinity Audio <= 5.21.0 - Information Exposure Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'cve', 'cve2025', 'wp-plugin', 'wordpress', 'trinity-audio', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        path = '/wp-content/plugins/trinity-audio/admin/inc/phpinfo.php'
        r = self.http_request(method='GET', path=path, allow_redirects=True)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('PHP Version', 'PHP Extension', 'trinity-audio',)
        if all(m in body for m in body_all):
            self.set_info(severity='medium', reason='Trinity Audio <= 5.21.0 - Information Exposure detected', path=path)
            return True
        return False
