...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Detects Datadog Browser RUM (Real User Monitoring) SDK artifacts in HTML responses.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'rum', 'datadog'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('DD_RUM.init', 'DD_RUM.onReady',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Datadog Browser RUM detected",
                path='/',
            )
            return True
        return False
