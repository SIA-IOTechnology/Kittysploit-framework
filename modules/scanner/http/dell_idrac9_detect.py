...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/sysmgmt/2015/bmc/info', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('BuildVersion', 'SystemModelName',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Detect Dell iDRAC9 detected",
