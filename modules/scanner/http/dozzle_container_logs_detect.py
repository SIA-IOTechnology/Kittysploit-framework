...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Dozzle - Logs Exposure Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'exposure', 'logs', 'dozzle', 'docker', 'vuln'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://github.com/amir20/dozzle', 'https://dozzle.dev/'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('"authorizationNeeded": "false"', 'Dozzle',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="Dozzle - Logs Exposure detected",
                path='/',
            )
            return True
        return False
