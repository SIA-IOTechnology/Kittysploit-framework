...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://dradisframework.com/ce/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/pro/login', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Dradis Professional Edition Login Panel detected",
                path='/pro/login',
            )
            return True
        return False
