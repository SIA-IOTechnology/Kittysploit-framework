#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""The index contains the list of exposed Java classes."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'low',
        'tags': ['web', 'scanner', 'technology', 'tech', 'misconfig', 'dwr'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['http://aetherlab.net/2012/09/experiences-in-pentesting-dwr/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/dwr/index.html', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('<title>DWR Test Index</title>',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='low',
                reason="DWR detect test page detection detected",
                path='/dwr/index.html',
            )
            return True
        return False
