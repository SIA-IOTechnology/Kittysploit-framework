...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'eclipsebirt'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://eclipse.github.io/birt-website/'],
    }

    def run(self):
        for path in ('/', '/reportviewer/', '/birt/', '/birt-viewer/'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="Eclipse BIRT Panel detected",
                    path=path,
                )
                return True
        return False
