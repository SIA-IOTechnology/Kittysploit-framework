...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'elfinder'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/', '/elfinder.html'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_markers = ('<!-- Element where elFinder will be created (REQUIRED) -->',)
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="elFinder - Install detected",
                    path=path,
                )
                return True
        return False
