...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Espocrm Installer Detection',
        'description': 'Espocrm is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'espocrm', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>EspoCRM Installation',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Espocrm Installer detected",
                path='/install/',
            )
            return True
        return False
