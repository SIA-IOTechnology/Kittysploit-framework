...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'exolis', 'engage'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.exolis.fr/en/solution-2/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/app.js', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Exolis Engage Panel detected",
                path='/app.js',
            )
            return True
        return False
