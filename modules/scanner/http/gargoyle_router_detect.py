...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'iot', 'gargoyle', 'router', 'edb'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.exploit-db.com/ghdb/8004'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/login.sh', allow_redirects=False)
        ...
        if body_hit:
            self.set_info(
                severity='info',
                reason="Gargoyle Router Management Utility Admin Login Panel detected",
                path='/login.sh',
            )
            return True
        return False
