...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'panos', 'globalprotect', 'paloaltonetworks'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        for path in ('/global-protect/login.esp', '/sslmgr'):
            ...
            if any(m in body for m in markers):
                self.set_info(
                    severity='info',
                    reason="Palo Alto Networks GlobalProtect Login Panel detected",
                    path=path,
                )
                return True
        return False
