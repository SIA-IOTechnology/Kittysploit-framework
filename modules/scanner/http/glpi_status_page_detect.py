...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'A php status page that indicates if local or ldap identity is used for glpi.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'status', 'glpi'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('GLPI_DB_OK', 'GLPI_SESSION_DIR_OK', 'GLPI_OK',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="GLPI Status Page detected",
                path='/status.php',
            )
            return True
        return False
