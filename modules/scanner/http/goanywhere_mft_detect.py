...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'goanywhere', 'login', 'filetransfer'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        for path in ('/goanywhere/auth/Login.xhtml', '/webclient/Login.xhtml'):
            ...
            if any(m in body for m in markers):
                self.set_info(
                    severity='info',
                    reason="GoAnywhere Managed File Transfer Login Panel detected",
                    path=path,
                )
                return True
        return False
