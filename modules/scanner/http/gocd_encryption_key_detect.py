...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'GoCd Encryption Key Detection',
        'description': 'GoCd Encryption Key is exposed.',
        'author': ['KittySploit Team'],
        'severity': 'low',
        'tags': ['web', 'scanner', 'misconfiguration', 'go', 'gocd', 'exposure', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/go/add-on/business-continuity/api/cipher.aes', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        header_any = ('text/plain',)
        body_regexes = ('([a-z0-9]){32}',)
        if (any(m in headers for m in header_any)) and (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='low',
                reason="GoCd Encryption Key detected",
