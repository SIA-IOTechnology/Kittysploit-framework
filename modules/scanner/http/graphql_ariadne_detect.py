#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Detects Graphql Ariadne Detect."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Detects Graphql Ariadne Detect.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'graphql', 'ariadne'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/graphql', '/api/graphql', '/query', '/'):
            r = self.http_request(
                ...
                data='{"query":"query { __typename @abc }"}',
            )
            ...
            if "Unknown directive '@abc'." not in body:
                continue
            ...
            self.set_info(
                severity='info',
                reason='Graphql Ariadne detected',
                path=path,
            )
            return True
        return False
