...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'H2O Dashboard - Exposure Detection',
        'description': 'H2o dashboard by default has no authentication and can lead to RCE on the host.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'exposure', 'h2o', 'ml', 'unauth'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_any = ('H2O Flow',)
        header_all = ('X-H2o-Build-Project-Version', 'X-H2o-Cluster-Id',)
        if (any(m in body for m in body_any)) and (all(m in headers for m in header_all)):
            self.set_info(
                severity='high',
                reason="H2O Dashboard - Exposure detected",
                path='/',
            )
            return True
        return False
