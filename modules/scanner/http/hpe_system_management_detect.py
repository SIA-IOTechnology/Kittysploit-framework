...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'hpe', 'tech'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/cpqlogin.htm', allow_redirects=False)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Hewlett Packard Enterprise System Management Login Panel detected",
                path='/cpqlogin.htm',
            )
            return True
        return False
