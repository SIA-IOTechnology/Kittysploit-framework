...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/', '/favicon.ico', '/modules/ibm_apim/ibm_apim.info.yml'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_any = ('ibm developer portal is ready', 'package: ibm api developer',)
            mmh3_vals = ('801517258',)
            if (any(m in body for m in body_any)) and (shodan_mmh3(r.content or b"") in mmh3_vals):
                self.set_info(
                    severity='info',
                    reason="IBM API Connect Developer Portal detected",
                    path=path,
                )
                return True
        return False
