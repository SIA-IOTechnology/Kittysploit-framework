...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'websphere', 'ibm'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Administrative Console Login', 'Log in to the Administrative Console',)
        if all(m in body for m in body_all):
            self.set_info(
                severity='info',
                reason='IBM WebSphere Application Server Community Edition Admin Login Panel detected',
                path=path,
            )
            return True
        return False
