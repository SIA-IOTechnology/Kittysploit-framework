#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""IceWarp open redirect vulnerabilities were detected."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'IceWarp - Open Redirect Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'icewarp', 'redirect', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if (any(m in headers for m in header_any)) and (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="IceWarp - Open Redirect detected",
                path='///interact.sh/%2F..',
            )
            return True
        return False
