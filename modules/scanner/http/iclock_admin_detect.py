...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'iclock', 'login', 'admin'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/iclock/accounts/login/', '/iclock/accounts/login/?next=/iclock/data/iclock/'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="iClock Automatic Data Master Server Admin Panel detected",
                    path=path,
                )
                return True
        return False
