...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Indegy Sensor Setup - Installer Detection',
        'description': 'Indegy Sensor is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'indegy', 'sensor', 'installer', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Setup Wizard</title>',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Indegy Sensor Setup - Installer detected",
                path='/settings',
            )
            return True
        return False
