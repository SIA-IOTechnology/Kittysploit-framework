...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/cp/Shares?user=&protocol=webaccess&v=2.3', allow_redirects=False)
        ...
        if body_hit and any(m in headers for m in header_markers):
            self.set_info(
                severity='info',
                reason="Iomega LenovoEMC NAS Login Panel detected",
