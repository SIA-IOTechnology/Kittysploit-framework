...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'jaspersoft', 'tibco'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        for path in ('/jasperserver/login.html?error=1', '/jasperserver-pro/login.html?error=1'):
            ...
            if any(m in body for m in markers):
                self.set_info(
                    severity='info',
                    reason="TIBCO Jaspersoft Login Panel detected",
                    path=path,
                )
                return True
        return False
