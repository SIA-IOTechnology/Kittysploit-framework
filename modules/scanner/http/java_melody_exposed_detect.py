...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'JavaMelody Monitoring Exposed Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'config', 'java', 'javamelody', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/monitoring', '/..%3B/monitoring'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_any = ('Monitoring JavaMelody on',)
            if (any(m in body for m in body_any)):
                self.set_info(
                    severity='medium',
                    reason="JavaMelody Monitoring Exposed detected",
                    path=path,
                )
                return True
        return False
