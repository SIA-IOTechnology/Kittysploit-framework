...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'jsf', 'tech', 'primefaces', 'richfaces'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('javax.faces.resource', 'javax.faces.ViewState', 'primefaces', 'richfaces',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="JavaServer Faces detected",
                path='/',
            )
            return True
        return False
