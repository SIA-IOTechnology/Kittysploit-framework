...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'kafka', 'apache', 'provectus'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://kafka.apache.org/', 'https://github.com/provectus/kafka-ui'],
    }
    ...
    def run(self):
        for path in ('/', '/info'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="Apache Kafka Topics Panel detected",
                    path=path,
                )
                return True
        return False
