...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Home router wireless KLR 300N login panel were Detected.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'keo', 'klr300n', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = (r.text or "")
        markers = ('<title>Roteador Wireless KLR 300N</title>',)
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="KLR 300N Router Panel detected",
                path='/login.asp',
            )
            return True
        return False
