...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'kopano', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/webapp/', allow_redirects=True)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('<title>Kopano WebApp',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Kopano WebApp Login Panel detected",
                path='/webapp/',
            )
            return True
        return False
