...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/wfc/portal', '/wfc/logon'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_regexes = ('Kronos', 'Workforce Central(R)',)
            if any(re.search(rx, body, 0) for rx in body_regexes):
                self.set_info(
                    severity='info',
                    reason="Kronos Workforce Central Login Panel detected",
                    path=path,
                )
                return True
        return False
