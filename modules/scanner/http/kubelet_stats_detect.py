...
class Module(Scanner, Http_client):
    ...
    def run(self):
        r = self.http_request(method="GET", path='/stats/summary', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('"node":', '"nodeName":',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Kubelet Stats detected",
                path='/stats/summary',
            )
            return True
        return False
