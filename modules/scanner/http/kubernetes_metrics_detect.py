...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'low',
        'tags': ['web', 'scanner', 'misconfiguration', 'kubernetes', 'exposure', 'devops', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('namespace', 'HELP', 'TYPE', 'kube',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='low',
                reason="Detect Kubernetes Exposed Metrics detected",
                path='/metrics',
            )
            return True
        return False
