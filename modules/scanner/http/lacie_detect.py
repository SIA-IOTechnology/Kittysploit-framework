...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'lacie', 'login', 'edb'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.exploit-db.com/ghdb/7118'],
    }
    ...
    def run(self):
        for path in ('/', '/dashboard/'):
            ...
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_markers = ('id_LaCie',)
            body_regexes = ('(?m)<title>([a-zA-Z0-9&#; ]|)+Dashboard<\\/title>$',)
            if any(m in body for m in body_markers) and any(re.search(rx, body, 0) for rx in body_regexes):
                self.set_info(
                    severity='info',
                    reason="LaCie Login Panel detected",
                    path=path,
                )
                return True
        return False
