...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'liferay', 'portal'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.liferay.com/', 'https://github.com/mzer0one/CVE-2020-7961-POC'],
    }

    def run(self):
        for path in ('/', '/api/jsonws', '/api/jsonws/invoke'):
            ...
            if body_hit and any(m in headers for m in header_markers):
                self.set_info(
                    severity='info',
                    reason="Liferay Login Panel detected",
                    path=path,
                )
                return True
        return False
