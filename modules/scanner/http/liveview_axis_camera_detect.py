...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://www.exploit-db.com/ghdb/6843'],
    }

    def run(self):
        for path in ('/view/viewer_index.shtml', '/pics/logo_70x29px.gif'):
            r = self.http_request(method="GET", path=path, allow_redirects=True)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_any = ('live view  - axis', 'live view - axis',)
            mmh3_vals = ('1106649083',)
            if (any(m in body for m in body_any)) and (shodan_mmh3(r.content or b"") in mmh3_vals):
                self.set_info(
                    severity='info',
                    reason="AXIS Network Camera Live View detected",
                    path=path,
                )
                return True
        return False
