...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Lvmeng - UTS Disclosure Detection',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/webapi/v1/system/accountmanage/account', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('password', 'nsfocus_uts', 'MANAGER_IP',)
        header_all = ('application/json',)
        if (all(m in body for m in body_all)) and (all(m in headers for m in header_all)):
            self.set_info(
                severity='high',
                reason="Lvmeng - UTS Disclosure detected",
