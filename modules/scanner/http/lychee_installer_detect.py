...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Lychee Installer Detection',
        'description': 'Lychee is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'lychee', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_any = ('Lychee Installer',)
        header_any = ('text/html',)
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Lychee Installer detected",
                path='/install',
            )
            return True
        return False
