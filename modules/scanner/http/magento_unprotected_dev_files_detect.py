...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Magento Unprotected development files Detection',
    ...
    def run(self):
        for path in ('/dev/tests/functional/credentials.xml.dist', '/dev/tests/functional/etc/config.xml.dist'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items()).lower()
            body_any = ('magento', 'replace xmlns:xsi=', '<field path=', 'config xmlns:xsi', '<application>', '<install>',)
            header_any = ('application/xml', 'application/octet-stream',)
            if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
                self.set_info(
                    severity='high',
                    reason="Magento Unprotected development files detected",
                    path=path,
                )
                return True
        return False
