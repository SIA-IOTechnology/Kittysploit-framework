...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'zoho', 'manageengine'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.manageengine.com/key-manager/'],
    }

    def run(self):
        for path in ('/apiclient/index.jsp', '/pki/images/keyManager_title.ico'):
            r = self.http_request(method='GET', path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_any = ('<title>key manager plus</title>',)
            mmh3_vals = ('192917117',)
            if (any(m in body for m in body_any)) and (shodan_mmh3(r.content or b"") in mmh3_vals):
                self.set_info(
                    severity='info',
                    reason='ZOHO ManageEngine KeyManagerPlus Panel detected',
                    path=path,
                )
                return True
        return False
