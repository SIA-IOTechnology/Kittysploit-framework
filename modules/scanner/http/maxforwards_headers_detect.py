#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Max-Forwards response header is specified."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'misc', 'miscellaneous', 'max-forwards', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        header_any = ('max-forwards:',)
        if (any(m in headers for m in header_any)):
            self.set_info(
                severity='info',
                reason="Max-Forwards Header detected",
                path='/',
            )
            return True
        return False
