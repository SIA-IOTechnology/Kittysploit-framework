...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'microfocus', 'vibe'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/ssf/s/portalLogin', allow_redirects=True)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Micro Focus Vibe Login Panel detected",
