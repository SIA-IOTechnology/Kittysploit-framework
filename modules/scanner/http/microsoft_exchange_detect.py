...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'microsoft', 'exchange'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/owa/auth/logon.aspx?replaceCurrent=1&url=/ecp', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Microsoft Exchange Admin Center Login Panel detected",
