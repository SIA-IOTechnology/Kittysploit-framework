#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Some Microsoft IIS servers have the version on the response header."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'microsoft', 'iis'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if any(m in headers for m in header_markers):
            self.set_info(
                severity='info',
                reason="Microsoft IIS version detect detected",
                path='/',
            )
            return True
        return False
