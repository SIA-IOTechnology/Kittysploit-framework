...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Mixed Active Content Detection',
        'description': 'This check detects if there are any active content loaded over HTTP instead of HTTPS.',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_regexes = ('(?mi)<!--\\[if (lt|lte) IE [0-9]*\\]>\\s*<script[^>]*\\ssrc="http://', '<!--\\s*<script', '<script[^>]*src=[\'"]http://[^\'">]+[\'"]', '<iframe[^>]*src=[\'"]http://[^\'">]+[\'"]', '<object[^>]*data=[\'"]http://[^\'">]+[\'"]',)
        if any(re.search(rx, body) for rx in body_regexes):
            self.set_info(
                severity='info',
                reason='Mixed Active Content detected',
                path=path,
            )
            return True
        return False
