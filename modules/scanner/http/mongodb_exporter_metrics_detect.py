...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'mongodb', 'exposure', 'debug', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('# HELP', 'mongodb', 'mongodb_exporter_build_info',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='medium',
                reason="MongoDB Exporter detected",
                path='/metrics',
            )
            return True
        return False
