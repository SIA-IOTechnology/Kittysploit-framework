...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Moodle Installation Exposure Detection',
        'description': 'Moodle is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'moodle', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_any = ('Installation - Moodle',)
        header_any = ('text/html',)
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Moodle Installation Exposure detected",
                path='/install.php',
            )
            return True
        return False
