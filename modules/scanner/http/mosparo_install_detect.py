...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'mosparo Exposed Installation Detection',
        'description': 'mosparo is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'mosparo', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Setup - mosparo',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="mosparo Exposed Installation detected",
                path='/setup/',
            )
            return True
        return False
