#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A cross-platform, post-exploit, red teaming framework built with python3, docker, docker-compose, and a web br."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'c2', 'ir', 'osint', 'mythic', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Mythic</title><',)
        if any(m in body for m in body_any):
            self.set_info(
                severity='info',
                reason='Mythic C2 detected',
                path=path,
            )
            return True
        return False
