#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""3CX Phone System Web Client Management Console panel was detected."""

from kittysploit import *
from lib.protocols.http.http_client import Http_client


class Module(Scanner, Http_client):
    __info__ = {
        'name': '3CX Phone System Web Client Management Console - Panel Detect',
        'description': '3CX Phone System Web Client Management Console panel was detected.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', '3cx'],
        'agent': {
            'risk': 'active',
            'effects': ['network_probe'],
            'expected_requests': 1,
            'reversible': True,
            'approval_required': False,
            'produces': ['tech_hints', 'risk_signals', 'endpoints'],
            'cost': 1.0,
            'noise': 0.3,
            'value': 1.0,
            'requires': {
                'min_endpoints': 0,
                'min_params': 0,
                'tech_hints_any': [],
                'tech_hints_all': [],
                'specializations_any': [],
                'risk_signals_any': [],
                'auth_session': False,
                'capabilities_any': [],
                'capabilities_all': [],
                'confidence_min': {
                },
                'confidence_min_any': {
                },
                'endpoint_pattern_any': [],
                'param_any': [],
                'api_surface_ready': False,
            },
            'chain': {
                'produces_capabilities': [
                    {
                        'capability': 'admin_surface',
                        'from_detail': '',
                    },
                ],
                'consumes_capabilities': [],
                'option_bindings': {
                },
                'suggested_followups': ['auxiliary/scanner/http/login_page_detector'],
            },
        },
        'references': [
            'https://www.3cx.com/phone-system/',
            'https://www.3cx.com/blog/unified-communications/client-apps/',
            'https://medium.com/@frycos/pwning-3cx-phone-management-backends-from-the-internet-d0096339dd88',
        ],
    }

    def run(self):
        r = self.http_request(method="GET", path='/webclient/', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = (r.text or "")
        markers = (
            '<title>3CX Webclient</title>',
        )
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="3CX Phone System Web Client Management Console - Panel Detect detected",
                path='/webclient/',
            )
            return True
        return False

