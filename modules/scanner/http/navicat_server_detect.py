#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Navicat On-Prem Server is an on-premise solution that provides you with the option to host a cloud environment."""
    ...
    def run(self):
        ...
        for path in ('/', '/login'):
            ...
            if any(m in body for m in markers):
                self.set_info(
                    severity='info',
                    reason="Navicat On-Prem Server Panel detected",
                    path=path,
                )
                return True
        return False
