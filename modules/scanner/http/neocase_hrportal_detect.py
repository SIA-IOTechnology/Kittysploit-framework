...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Neocase HR Portal login panel was detected.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'neocase', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_markers = ('neocase', 'hr portal')
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Neocase HR Portal Login Panel detected",
                path='/',
            )
            return True
        return False
