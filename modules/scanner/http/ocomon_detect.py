...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'ocomon', 'oss', 'ocomon_project'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://sourceforge.net/projects/ocomonphp/'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('open_form/ticket_form_open.php',)
        body_regexes = ('(?i)(OcoMon)',)
        if any(m in body for m in body_markers) and any(re.search(rx, body, 0) for rx in body_regexes):
            self.set_info(
                severity='info',
                reason="OcoMon Login Panel detected",
                path='/',
            )
            return True
        return False
