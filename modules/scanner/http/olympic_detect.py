...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'olympic', 'login'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.olympicbankingsystem.com/en/'],
    }

    def run(self):
        for path in ('/Connect.do', '/javaScript/responsive/portal.js'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="OLYMPIC Banking System Login Panel detected",
                    path=path,
                )
                return True
        return False
