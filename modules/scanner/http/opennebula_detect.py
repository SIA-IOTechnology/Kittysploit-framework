...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'opennebula'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://opennebula.io/', 'https://github.com/OpenNebula'],
    }
    ...
    def run(self):
        ...
        body_markers = (
            '<title>OpenNebula',
            'OpenNebula Systems',
            'target="_blank">OpenNebula',
        )
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="OpenNebula Login Panel detected",
                path='/',
            )
            return True
        return False
