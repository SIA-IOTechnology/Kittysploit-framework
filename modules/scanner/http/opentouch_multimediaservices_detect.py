...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'opentouch', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/authenticationform/login', '/authenticationform/jsp/logonWeb.jsp'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="OpenTouch Multimedia Services detected",
                    path=path,
                )
                return True
        return False
