...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://www.oracle.com/security/identity-management/access-management/'],
    }

    def run(self):
        for path in ('/oam/pages/login.jsp', '/'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="Oracle Access Management Login Panel detected",
                    path=path,
                )
                return True
        return False
