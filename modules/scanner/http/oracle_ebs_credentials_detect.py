...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'exposure', 'config', 'oracle', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/OA_HTML/jtfwrepo.xml', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_any = ('password=', '<PUSR_LIST>',)
        header_any = ('text/xml',)
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Oracle E-Business System Credentials Page detected",
