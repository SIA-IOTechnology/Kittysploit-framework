...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://kb.orbitgt.com/214/technology/platforms/index'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('<title>Orbit Telephone System: Administration</title>',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Orbit Telephone System detected",
                path='/index.html',
            )
            return True
        return False
