...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'panel', 'fleet', 'osquery', 'tech'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('Fleet', 'osquery', '/assets/bundle',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="OSQuery Fleet Panel detected",
                path='/login',
            )
            return True
        return False
