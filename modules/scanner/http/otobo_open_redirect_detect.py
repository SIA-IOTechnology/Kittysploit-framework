#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Otobo contains an open redirect vulnerability."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Otobo - Open Redirect Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'redirect', 'otobo', 'huntr', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/otobo/index.pl?Action=ExternalURLJump;URL=http://www.interact.sh', allow_redirects=False)
        ...
        if (any(re.search(rx, headers, 0) for rx in header_regexes)):
            self.set_info(
                severity='medium',
                reason="Otobo - Open Redirect detected",
