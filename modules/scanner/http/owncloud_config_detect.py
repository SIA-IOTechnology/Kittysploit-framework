...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://owncloud.com/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/owncloud/config/', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Index of', 'owncloud/config',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='info',
                reason="owncloud Config detected",
