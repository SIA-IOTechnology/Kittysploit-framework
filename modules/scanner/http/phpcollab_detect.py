...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'phpcollab', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/general/login.php', allow_redirects=False)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="phpCollab Login Panel detected",
