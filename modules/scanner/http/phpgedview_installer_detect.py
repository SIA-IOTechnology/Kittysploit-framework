...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'PhpGedView Installer Exposure Detection',
        'description': 'PhpGedView is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'phpgedview', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('installation wizard', 'phpgedview',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='high',
                reason="PhpGedView Installer Exposure detected",
                path='/install.php',
            )
            return True
        return False
