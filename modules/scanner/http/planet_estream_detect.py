...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'planet', 'estream', 'planetestream'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/Login.aspx', allow_redirects=True)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Planet eStream Login Panel detected",
                path='/Login.aspx',
            )
            return True
        return False
