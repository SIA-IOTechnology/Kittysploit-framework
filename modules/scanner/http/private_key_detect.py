...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'exposure', 'node', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/node_modules/mqtt/test/helpers/', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Index of /node_modules/mqtt/test/helpers', 'Parent Directory',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='high',
                reason="Private key exposure via helper detector detected",
