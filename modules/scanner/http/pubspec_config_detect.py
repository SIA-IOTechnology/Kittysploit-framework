...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'exposure', 'devops', 'pubsec', 'config', 'cicd', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/pubspec.yaml', '/assets/pubspec.yaml'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_all = ('version:', 'environment:', 'dependencies:',)
            if (all(m in body for m in body_all)):
                self.set_info(
                    severity='info',
                    reason="Pubspec YAML Configuration File detected",
                    path=path,
                )
                return True
        return False
