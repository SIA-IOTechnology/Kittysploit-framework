...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'QuickCMS Installation Wizard Detection',
        'description': 'PMM is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'exposure', 'install', 'quickcms', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('QuickCMS Installation',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="QuickCMS Installation Wizard detected",
                path='/install/',
            )
            return True
        return False
