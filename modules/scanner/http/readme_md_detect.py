...
class Module(Scanner, Http_client):
    ...
    def run(self):
        ...
        r = self.http_request(method="GET", path='/README.md', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_any = ('#', '##',)
        header_any = ('text/markdown', 'text/plain',)
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='info',
                reason="README.md file disclosure detected",
                path='/README.md',
            )
            return True
        return False
