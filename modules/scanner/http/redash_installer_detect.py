...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Redash Installer Exposure Detection',
        'description': 'Redash is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'redash', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('Redash Initial Setup', 'Admin User',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Redash Installer Exposure detected",
                path='/setup',
            )
            return True
        return False
