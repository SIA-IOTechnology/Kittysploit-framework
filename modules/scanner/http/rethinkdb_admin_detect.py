...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'rethinkdb', 'console', 'dashboard', 'vuln'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://rethinkdb.com/'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('RethinkDB Administration Console',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='medium',
                reason="RethinkDB Administration Console detected",
                path='/#dashboard',
            )
            return True
        return False
