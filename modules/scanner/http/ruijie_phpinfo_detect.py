...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'low',
        'tags': ['web', 'scanner', 'exposure', 'phpinfo', 'rujjie', 'config', 'ruijie', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/tool/view/phpinfo.view.php', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('PHP Version', 'PHP Extension',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='low',
                reason="Ruijie Phpinfo Configuration detected",
