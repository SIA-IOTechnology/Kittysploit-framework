#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Detected SAP systems where the SAPControl SOAP web service exposes the ABAPReadSyslog operation without authen."""
...
class Module(Scanner, Http_client):
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('<item>', '<Time>', '<Text>', 'ABAPReadSyslogResponse',)
        if all(m in body for m in body_all):
            self.set_info(severity='medium', reason='SAPControl ABAPReadSyslog - Disclosure detected', path=path)
            return True
        return False
