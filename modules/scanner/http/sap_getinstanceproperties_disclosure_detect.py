#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Detected SAP systems where the SAP Start Service (sapstartsrv) SAPControl SOAP interface exposes the GetInstan."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SAPControl Webmethods - Disclosure Detection',
        'description': 'Detected SAP systems where the SAP Start Service (sapstartsrv) SAPControl SOAP interface exposes the GetInstanceProperties method without requiring authentication.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfig', 'sap', 'sapcontrol', 'soap', 'exposure'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('GetInstancePropertiesResponse', '<property>Webmethods</property>',)
        if all(m in body for m in body_all):
            self.set_info(severity='medium', reason='SAPControl Webmethods - Disclosure detected', path=path)
            return True
        return False
