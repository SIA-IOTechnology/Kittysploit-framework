...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'sap', 'tech', 'igs'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('SAP IGS', 'is running',)
        body_word_hit = any(m in body for m in body_markers)
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        header_markers = ('SAP Internet Graphics Server',)
        if any(m in headers for m in header_markers):
            self.set_info(
                severity='info',
                reason="SAP Internet Graphics Server (IGS) detected",
                path='/',
            )
            return True
        return False
