#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Detected SAP systems where the SAP Start Service (sapstartsrv) SAPControl SOAP interface exposes the ListLogFi."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SAPControl ListLogFiles - Disclosure Detection',
        'description': 'Detected SAP systems where the SAP Start Service (sapstartsrv) SAPControl SOAP interface exposes the ListLogFiles web method without authentication.',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('ListLogFilesResponse',)
        body_all = ('<file>', '<filename>',)
        if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
            self.set_info(severity='medium', reason='SAPControl ListLogFiles - Disclosure detected', path=path)
            return True
        return False
