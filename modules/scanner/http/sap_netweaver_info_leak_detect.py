...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SAP NetWeaver ICM Info page leak Detection',
        'description': 'Detection of SAP NetWeaver ABAP Webserver /public/info page',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'sap', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/sap/public/info', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_regexes = ('RFC_SYSTEM_INFO.Response',)
        if (any(re.search(rx, body, 0) for rx in body_regexes)):
            self.set_info(
                severity='medium',
                reason="SAP NetWeaver ICM Info page leak detected",
