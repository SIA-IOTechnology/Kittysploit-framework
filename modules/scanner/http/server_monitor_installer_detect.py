...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Server Monitor Installer Detection',
        'description': 'Server Monitor is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'monitor', 'exposure', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('<title>SERVER MONITOR - Install</title>', 'PHP Server Monitor - Install', 'install.php?action=config',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Server Monitor Installer detected",
                path='/install.php',
            )
            return True
        return False
