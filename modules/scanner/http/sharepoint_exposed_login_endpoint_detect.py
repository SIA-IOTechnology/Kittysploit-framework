...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Microsoft SharePoint - Exposed Login Endpoint Detection',
        'description': 'Detected Microsoft SharePoint login and authentication endpoints.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'misconfiguration', 'sharepoint', 'microsoft', 'login', 'exposure', 'misconfig', 'panel', 'tech'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/_layouts/15/Authenticate.aspx', '/_layouts/Authenticate.aspx'):
            ...
            if (any(re.search(rx, headers, 0) for rx in header_regexes)):
                self.set_info(
                    severity='info',
                    reason="Microsoft SharePoint - Exposed Login Endpoint detected",
                    path=path,
                )
                return True
        return False
