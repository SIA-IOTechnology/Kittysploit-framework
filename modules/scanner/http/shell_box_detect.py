#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Shell In A Box implements a web server that can export arbitrary command line tools to a web based terminal em."""
...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'shell', 'emulator', 'login', 'shellinabox_project'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Shell In A Box detected",
                path='/',
            )
            return True
        return False
