...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'tech', 'skycaiji'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/index.php?s=/admin/Index/index', allow_redirects=False)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="SkyCaiji Admin Panel detected",
                path='/index.php?s=/admin/Index/index',
            )
            return True
        return False
