...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'tech', 'slocum'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/sfmc/login', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Slocum Fleet Mission Control Login Panel detected",
                path='/sfmc/login',
            )
            return True
        return False
