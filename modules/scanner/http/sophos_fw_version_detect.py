...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/webconsole/webpages/login.jsp', '/userportal/webpages/myaccount/login.jsp'):
            ...
            if any(m in body for m in body_markers):
                self.set_info(
                    severity='info',
                    reason="Sophos Firewall Login Panel detected",
                    path=path,
                )
                return True
        return False
