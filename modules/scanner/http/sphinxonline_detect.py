...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'sphinxonline', 'login'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.lesphinx-developpement.fr/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/SphinxAuth/Account/Login', allow_redirects=False)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="SphinxOnline Panel detected",
