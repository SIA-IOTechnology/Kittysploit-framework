...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'spinnaker'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_markers = ('<spinnaker ng-if="!authenticating"></spinnaker>',)
        body_word_hit = any(m in body for m in body_markers)
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items()).lower()
        header_markers = ('x-spinnaker-priority',)
        if any(m in headers for m in header_markers):
            self.set_info(
                severity='info',
                reason="Spinnaker detected",
                path='/',
            )
            return True
        return False
