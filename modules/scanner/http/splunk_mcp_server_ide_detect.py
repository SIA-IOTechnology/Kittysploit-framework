...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'description': 'Splunk MCP Server IDE login interface was discovered.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'login', 'splunk', 'mcp', 'ide'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        markers = (
            ...
            '>Splunk MCP Server</h1>',
        )
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Splunk MCP Server IDE Login detected",
                path='/login',
            )
            return True
        return False
