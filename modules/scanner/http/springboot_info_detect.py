...
class Module(Scanner, Http_client):
    ...
    def run(self):
        ...
        for path in ('/info', '/actuator/info', '/management', '/management/info'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_any = ('application/json', 'application/vnd.spring-boot', '"os"', '"runtime"', '"jvm"', '"build"', '"vendor"',)
            body_all = ()
            if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
                self.set_info(
                    severity='info',
                    reason="Spring Boot Information Panel detected",
                    path=path,
                )
                return True
        return False
