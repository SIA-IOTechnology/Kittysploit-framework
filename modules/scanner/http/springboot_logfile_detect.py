...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/logfile', '/actuator/logfile', '/actuators/logfile'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
            body_all = ('springframework.web.HttpRequestMethodNotSupportedException', 'INFO',)
            header_any = ('text/plain',)
            if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
                self.set_info(
                    severity='low',
                    reason="Detects Springboot Logfile Actuator detected",
                    path=path,
                )
                return True
        return False
