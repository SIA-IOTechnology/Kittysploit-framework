...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'subrion', 'intelliants'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/panel', allow_redirects=True)
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="Subrion Admin Panel Login Panel detected",
                path='/panel',
            )
            return True
        return False
