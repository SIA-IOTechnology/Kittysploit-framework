...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SugarCRM Exposed Installation Detection',
        'description': 'SugarCRM is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'sugar', 'sugarcrm', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Sugar Setup Wizard',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="SugarCRM Exposed Installation detected",
                path='/install.php',
            )
            return True
        return False
