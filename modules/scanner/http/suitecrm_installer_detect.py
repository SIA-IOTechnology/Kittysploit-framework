...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SuiteCRM Installer Exposure Detection',
        'description': 'SuiteCRM is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'suitecrm', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('SuiteCRM Setup Wizard:', 'Checking Environment',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="SuiteCRM Installer Exposure detected",
                path='/install.php',
            )
            return True
        return False
