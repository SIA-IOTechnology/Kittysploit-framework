...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SumoWebTools Installer Exposure Detection',
        'description': 'SumoWebTools is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'sumowebtools', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('SumoWebTools Installer', 'configure',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="SumoWebTools Installer Exposure detected",
                path='/install',
            )
            return True
        return False
