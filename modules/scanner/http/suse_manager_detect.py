...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SUSE Manager Server - Panel Detection',
        'description': 'SUSE Manager login panel detected.',
        'author': ['KittySploit Team'],
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'suse', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="SUSE Manager Server detected",
                path='/',
            )
            return True
        return False
