...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'SVN wc.db File Exposure Detection',
        'description': 'SVN wc.db file is exposed.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'exposure', 'msf', 'svn', 'config', 'files', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        for path in ('/.svn/wc.db', '/wc.db'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_all = ('SQLite format', 'WCROOT',)
            if (all(m in body for m in body_all)):
                self.set_info(
                    severity='medium',
                    reason="SVN wc.db File Exposure detected",
                    path=path,
                )
                return True
        return False
