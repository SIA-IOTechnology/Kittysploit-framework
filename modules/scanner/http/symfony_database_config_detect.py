...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://symfony.com/legacy/doc/reference/1_3/en/07-Databases'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/config/databases.yml', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
        body_all = ('class:', 'param:',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Symfony Database Configuration File detected",
