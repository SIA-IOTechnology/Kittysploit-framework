...
class Module(Scanner, Http_client):
    ...
    def run(self):
        for path in ('/config/packages/security.yaml', '/app/config/security.yml'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_all = ('security:', 'firewalls:', 'access_control:',)
            if (all(m in body for m in body_all)):
                self.set_info(
                    severity='info',
                    reason="Symfony Security Configuration File detected",
                    path=path,
                )
                return True
        return False
