...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Tasmota Installer Exposure Detection',
        'description': 'Tasmota is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'tasmota', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('Install Tasmota', 'Tasmota Installer',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Tasmota Installer Exposure detected",
                path='/install/',
            )
            return True
        return False
