...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Tautulli - Exposed Installation Detection',
        'description': 'Tautulli is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'tautulli', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Tautulli - Welcome', 'Tautulli Setup Wizard',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='high',
                reason="Tautulli - Exposed Installation detected",
                path='/welcome',
            )
            return True
        return False
