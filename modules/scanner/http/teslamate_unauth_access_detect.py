...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'TeslaMate - Unauthenticated Access Detection',
        'description': 'A misconfig in Teslamate allows unauthorized access to /settings endpoint.',
        'author': ['KittySploit Team'],
        'severity': 'medium',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'teslamate', 'unauth', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Settings · TeslaMate', 'URLs</h2>',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="TeslaMate - Unauthenticated Access detected",
                path='/settings',
            )
            return True
        return False
