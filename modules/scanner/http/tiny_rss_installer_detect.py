...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Tiny Tiny RSS Installer Exposure Detection',
        'description': 'Tiny Tiny RSS is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'tiny', 'install', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('Tiny Tiny RSS - Installer', 'Test configuration',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Tiny Tiny RSS Installer Exposure detected",
                path='/install/',
            )
            return True
        return False
