...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'tracer', 'trane', 'iot', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/hui/index.html', allow_redirects=False)
        ...
        if body_hit:
            self.set_info(
                severity='info',
                reason="Tracer SC Login Panel detected",
                path='/hui/index.html',
            )
            return True
        return False
