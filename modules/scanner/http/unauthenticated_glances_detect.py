...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Glances Unauthenticated Panel Detection',
        'description': 'Glance running web server mode & Unauthenticated leads system monitoring to info disclosure',
        'author': ['KittySploit Team'],
        'severity': 'low',
        'tags': ['web', 'scanner', 'misconfiguration', 'exposure', 'glances', 'misconfig', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('<title>Monitor</title>',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='low',
                reason="Glances Unauthenticated Panel detected",
                path='/',
            )
            return True
        return False
