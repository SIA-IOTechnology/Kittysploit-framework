...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'unigui'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('uni-xtheme-', 'ext-all.js', 'uniVars._extVer=',)
        body_regexes = ('uni-(\\d+\\.\\d+\\.\\d+\\.\\d+)/', 'uniVars._extVer="(\\d+\\.\\d+\\.\\d+)"',)
        if any(m in body for m in body_markers) and any(re.search(rx, body, 0) for rx in body_regexes):
            self.set_info(
                severity='info',
                reason="UniGUI Framework detected",
                path='/',
            )
            return True
        return False
