...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'veritas', 'netbackup', 'opscenter', 'login'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://www.veritas.com/'],
    }

    def run(self):
        r = self.http_request(method="GET", path='/opscenter/', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="Veritas NetBackup OpsCenter Analytics Login detected",
                path='/opscenter/',
            )
            return True
        return False
