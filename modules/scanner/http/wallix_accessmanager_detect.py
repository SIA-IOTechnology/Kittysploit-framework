...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://www.wallix.com/privileged-access-management/access-manager/'],
    }

    def run(self):
        for path in ('/wabam', '/wabam/favicon.ico'):
            r = self.http_request(method="GET", path=path, allow_redirects=True)
            if not r or r.status_code != 200:
                continue
            body = (r.text or "").lower()
            body_any = ('<title>wallix access manager</title>',)
            mmh3_vals = ('1745235488',)
            if (any(m in body for m in body_any)) and (shodan_mmh3(r.content or b"") in mmh3_vals):
                self.set_info(
                    severity='info',
                    reason="Wallix Access Manager Panel detected",
                    path=path,
                )
                return True
        return False
