...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://github.com/imhunterand/ApachSAL/blob/main/assets/exploits.json'],
    }
    ...
    def run(self):
        for path in ('/web.config', '/../../web.config'):
            ...
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            body_all = ('<configuration>', '<system.webServer>',)
            if (all(m in body for m in body_all)):
                self.set_info(
                    severity='info',
                    reason="Web Configuration File detected",
                    path=path,
                )
                return True
        return False
