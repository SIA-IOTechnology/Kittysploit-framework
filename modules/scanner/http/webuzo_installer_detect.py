...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'Webuzo Installer Detection',
        'description': 'Webuzo is susceptible to the Installation page exposure due to misconfiguration.',
        'author': ['KittySploit Team'],
        'severity': 'high',
        'tags': ['web', 'scanner', 'misconfiguration', 'misconfig', 'webuzo', 'install', 'exposure', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        body_all = ('Webuzo Installer', 'Webuzo Initial Setup',)
        header_any = ('text/html',)
        if (all(m in body for m in body_all)) and (any(m in headers for m in header_any)):
            self.set_info(
                severity='high',
                reason="Webuzo Installer detected",
                path='/install.php',
            )
            return True
        return False
