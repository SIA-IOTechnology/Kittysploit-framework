...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'references': ['https://www.gnu.org/software/wget/manual/html_node/Wgetrc-Commands.html'],
    }

    def run(self):
        for path in ('/wgetrc', '/.wgetrc'):
            r = self.http_request(method="GET", path=path, allow_redirects=False)
            if not r or r.status_code != 200:
                continue
            body = r.text or ""
            headers = "\n".join(f"{k}: {v}" for k, v in r.headers.items())
            body_any = ('/etc/wgetrc', 'wget.info',)
            header_any = ('application/octet-stream',)
            if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
                self.set_info(
                    severity='info',
                    reason="Wgetrc Configuration File detected",
                    path=path,
                )
                return True
        return False
