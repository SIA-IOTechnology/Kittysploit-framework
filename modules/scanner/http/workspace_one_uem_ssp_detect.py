...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'panel', 'workspaceone', 'vmware', 'airwatch', 'ssp', 'login'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/MyDevice/Login', allow_redirects=False)
        ...
        if any(m in body for m in markers):
            self.set_info(
                severity='info',
                reason="VMware Workspace ONE UEM Airwatch Self-Service Portal detected",
                path='/MyDevice/Login',
            )
            return True
        return False
