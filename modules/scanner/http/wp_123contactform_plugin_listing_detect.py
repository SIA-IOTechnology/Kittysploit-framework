...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress 123ContactForm Plugin Directory Listing Detection',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/wp-content/plugins/123contactform-for-wordpress/', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('Index of', '/123contactform-for-wordpress',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='info',
                reason="WordPress 123ContactForm Plugin Directory Listing detected",
