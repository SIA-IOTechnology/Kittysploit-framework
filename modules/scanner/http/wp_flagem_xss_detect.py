#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WordPress FlagEm plugin contains a cross-site scripting vulnerability."""
...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'WordPress FlagEm - Cross-Site Scripting Detection',
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_any = ('/wp-content/plugins/FlagEm/',)
        ...
        if (any(m in body for m in body_any)) and (any(m in headers for m in header_any)):
            self.set_info(severity='high', reason='WordPress FlagEm - Cross-Site Scripting detected', path=path)
            return True
        return False
