...
class Module(Scanner, Http_client):
    __info__ = {
        'name': 'X Prober Server - Information Disclosure Detection',
    ...
    def run(self):
        r = self.http_request(method="GET", path='/xprober.php', allow_redirects=False)
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_all = ('"appName":"X Prober"', '<title>X Prober',)
        if (all(m in body for m in body_all)):
            self.set_info(
                severity='medium',
                reason="X Prober Server - Information Disclosure detected",
                path='/xprober.php',
            )
            return True
        return False
