...
class Module(Scanner, Http_client):
    ...
    def run(self):
        ...
        if not r or r.status_code != 500:
            return False
        body = r.text or ""
        body_any = ('<title>yii\\base\\ErrorException</title>',)
        if (any(m in body for m in body_any)):
            self.set_info(
                severity='info',
                reason="Yii Error Page - Detct detected",
                path='/',
            )
            return True
        return False
