...
class Module(Scanner, Http_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['web', 'scanner', 'technology', 'tech', 'yourls'],
        'agent': {
            'risk': 'active',
        ...
        'references': ['https://github.com/YOURLS/YOURLS'],
    }
    ...
    def run(self):
        ...
        if not r or r.status_code != 200:
            return False
        body = r.text or ""
        body_markers = ('<title>YOURLS', 'content="YOURLS', 'alt="YOURLS', '/yourls-logo',)
        if any(m in body for m in body_markers):
            self.set_info(
                severity='info',
                reason="YOURLSion detected",
                path='/',
            )
            return True
        return False
