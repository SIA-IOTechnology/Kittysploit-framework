...
class Module(Scanner, Tcp_scanner_client):
    __info__ = {
        ...
        'severity': 'info',
        'tags': ['scanner', 'tcp', 'network', 'telnet', 'bluecoat', 'proxy', 'detection'],
        'agent': {
            'risk': 'active',
    ...
    def run(self):
        ...
        if not data:
            return False
        if any(m.encode() in data for m in ('Blue Coat telnet proxy',)):
            self.set_info(severity='info', reason='BlueCoat Telnet Proxy detected')
            return True
        return False
